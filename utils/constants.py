...
ROW_PICK_OFFSETS = {
    ...
    7: {"x":  0.005, "y":  0.005}, # 第1横排(a1..h1)
}
# 逐排放置微调偏移（仅放置时叠加）
ROW_PLACE_OFFSETS = {
    # row: {"x": x偏移_m, "y": y偏移_m}
    0: {"x": 0.025, "y": 0.025}, # 第8横排(a8..h8)
    1: {"x":  0.015, "y":  0.015}, # 第7横排(a7..h7)
    2: {"x":  0.005, "y":  0.005}, # 第6横排(a6..h6)
...
# --- 远点夹爪倾斜参数 ---
# 距基座超过此距离(m)自动倾斜，最大倾斜不超过 MAX_TILT_DEG 度
GRIPPER_TILT_THRESHOLD = 0.40   # 开始倾斜的距离阈值(m)
GRIPPER_MAX_TILT_DEG   = 30.0   # 最大倾斜角度(度)，垂直方向到倾斜方向的夹角
...
# 逐排 Z 轴偏移（按矩阵行索引, 覆盖全局 PICK_Z_OFFSET/PLACE_Z_OFFSET）
ROW_Z_OFFSET = {
    # row: Z偏移_m
    0: -0.003,   # 第8横排
    1: -0.003,   # 第7横排
